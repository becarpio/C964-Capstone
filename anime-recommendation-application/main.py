"""The Python coding for my Capstone Project from Western Governor's University.
Built using PyQT5, QT Designer, Python 3.8, PyCharm, Anaconda, Jupyter Notebooks,
and the My AnimeList Dataset (https://doi.org/10.34740/KAGGLE/DSV/45582).

This program uses Python via a combination of Jupyter Notebooks (to create the model) and PyCharm to recommend 5
anime to a user based on their selection in the window. There are also three graphs provided, that were requirements of
the final project.

For additional necessary files to run the application, please visit the github repo at
https://github.com/becarpio/C964-Capstone. Any additional required information should be contained in the Word document
found in the repo.
"""
...
# Import the scrubbed anime list
...
# UI information for the recommendation window. Window opens when "Recommendations" is clicked in the main window
class Ui_RecommendationWindow(object):
    def __init__(self):
        self.chosen_title = ""

    # Save which title the user selected
    def setChosenTitle(self, title):
        self.chosen_title = title

    # Return the title the user had selected
    def getChosenTitle(self):
        return self.chosen_title

    # Get 6 recommendations based on the user selected title (title 0 is thrown out as it is the same as the user
    # selection)
    ...
    # UI setup, code generated by QT Designer
...
# Code for the Main Window UI. Window opens when the program is run.
class Ui_MainWindow(QMainWindow):
    ...
    # Get the recommended similar titles. Causes the recommendation window to open and populate with the titles.
    ...
    # Initialize the ListView window to hold all anime titles
    ...
    # Update the ListView based on the text input bar when the filter button is clicked
    def updateListView(self):
        # Delete rows in current listview_model
        self.listview_model.removeRows(0, self.listview_model.rowCount())

        # If the text input box is blank, update the list to hold all the titles
        if self.main_txtinput.text() == "":
            for title in anime_list["title"]:
                item = QStandardItem(title)
                self.listview_model.appendRow(item)

        # Otherwise, if something is in the text input box, filter the information by the text
        else:
            for title in anime_list["title"]:
                if self.main_txtinput.text().lower() in title.lower():
                    item = QStandardItem(title)
                    self.listview_model.appendRow(item)
        # Check to see if Rows were found. If so, display the updated list.
        if self.listview_model.rowCount() > 0:
            # Show the model
            self.main_listview_animetitles.setModel(self.listview_model)
            self.main_listview_animetitles.setCurrentIndex(self.listview_model.index(0, 0))

        # If not, display a message box that shows no matches were found and re-initialize the ListView
        else:
            msg = QMessageBox()
    ...
    # UI setup, mostly generated by QT Designer with some additions
    ...
    # UI Setup provided by QT Designer
...
# Prepares the data from the imported anime_list to be accepted by the KNN Model
def prepData():
    ...
    # Transformed_anime_list needs to be accessed in several places and so needs to be a global variable.
